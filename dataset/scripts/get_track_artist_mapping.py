...
while start_index < len(df):
    ...
    for track in tracks:
        track_artist_mapping[track['id']] = ",".join([artist['id'] for artist in track['artists']])

    start_index = end_index


data = [[track_id, artists] for track_id, artists in track_artist_mapping.items()]
artists_df = pd.DataFrame(data, columns=['track_id','artist_ids'])
